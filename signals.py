...
def compute_scalp_signal(
    ...
    *,
    ...
    if tape_ready_long_bonus > 0:
        _add("LONG", "tape_ready", int(tape_ready_long_bonus), f"Tape-ready build (+{int(tape_ready_long_bonus)})")
    if vol_ok:
    ...
    if tape_ready_short_bonus > 0:
        _add("SHORT", "tape_ready", int(tape_ready_short_bonus), f"Tape-ready build (+{int(tape_ready_short_bonus)})")
    if vol_ok:
    ...
    # Tape mode: constructive compression / release readiness for chaotic tape.
    # This is intentionally behavior-based (tightening + hold + directional pressure)
    # rather than brittle pattern detection.
    tape_constructive_long = False
    tape_constructive_short = False
    tape_ready_long_score = 0
    tape_ready_short_score = 0
    tape_ready_long_bonus = 0
    tape_ready_short_bonus = 0
    if tape_mode and len(df) >= 8:
        try:
            rr = (df["high"].astype(float) - df["low"].astype(float)).tail(6)
            rr_recent = float(rr.tail(3).mean()) if len(rr.tail(3)) else float(rr.mean())
            rr_prior = float(rr.head(3).mean()) if len(rr.head(3)) else rr_recent
            body = (df["close"].astype(float) - df["open"].astype(float)).abs().tail(6)
            body_recent = float(body.tail(3).mean()) if len(body.tail(3)) else float(body.mean())
            body_prior = float(body.head(3).mean()) if len(body.head(3)) else body_recent
            compression_tight = bool(rr_recent <= 0.96 * max(rr_prior, 1e-9) and body_recent <= 1.02 * max(body_prior, 1e-9))
            wick_ratio = float((((df["high"].astype(float) - df["low"].astype(float)) - (df["close"].astype(float) - df["open"].astype(float)).abs()) / (df["high"].astype(float) - df["low"].astype(float)).replace(0, np.nan)).tail(3).mean())
            wick_chop = bool(np.isfinite(wick_ratio) and wick_ratio >= 0.72 and not vol_ok)
            struct_hold_long = bool(micro_hl_pre or (float(df["low"].tail(3).min()) >= float(df["low"].tail(6).min())))
            struct_hold_short = bool(micro_lh_pre or (float(df["high"].tail(3).max()) <= float(df["high"].tail(6).max())))
            pressure_long = bool(macd_pre_long or macd_build_long_pts >= 3 or rsi_pre_long)
            pressure_short = bool(macd_pre_short or macd_build_short_pts >= 3 or rsi_pre_short)
            release_prox_long = bool(isinstance(ref_vwap, (float, int)) and isinstance(last_price, (float, int)) and isinstance(prox_abs, (float, int)) and float(last_price) <= float(ref_vwap) + max(float(prox_abs), 0.25 * float(atr_last or 0.0)))
            release_prox_short = bool(isinstance(ref_vwap, (float, int)) and isinstance(last_price, (float, int)) and isinstance(prox_abs, (float, int)) and float(last_price) >= float(ref_vwap) - max(float(prox_abs), 0.25 * float(atr_last or 0.0)))
            tape_ready_long_score = int(2 * compression_tight + 2 * struct_hold_long + 2 * pressure_long + 1 * release_prox_long - 2 * wick_chop)
            tape_ready_short_score = int(2 * compression_tight + 2 * struct_hold_short + 2 * pressure_short + 1 * release_prox_short - 2 * wick_chop)
            tape_constructive_long = bool(tape_ready_long_score >= 5)
            tape_constructive_short = bool(tape_ready_short_score >= 5)
            if tape_constructive_long and pressure_long:
                tape_ready_long_bonus = int(min(6, 2 + max(0, tape_ready_long_score - 4)))
            if tape_constructive_short and pressure_short:
                tape_ready_short_bonus = int(min(6, 2 + max(0, tape_ready_short_score - 4)))
        except Exception:
            tape_constructive_long = tape_constructive_short = False
            tape_ready_long_score = tape_ready_short_score = 0
            tape_ready_long_bonus = tape_ready_short_bonus = 0
    extras["tape_mode"] = bool(tape_mode)
    extras["tape_ready_long_score"] = int(tape_ready_long_score)
    extras["tape_ready_short_score"] = int(tape_ready_short_score)
    extras["tape_ready_long_bonus"] = int(tape_ready_long_bonus)
    extras["tape_ready_short_bonus"] = int(tape_ready_short_bonus)
    ...
    early_pre_long = bool(
        ...
        and ((macd_build_long_pts >= 4) or macd_pre_long or macd_event or tape_constructive_long)
    ...
    early_pre_short = bool(
        ...
        and ((macd_build_short_pts >= 4) or macd_pre_short or macd_event or tape_constructive_short)
    ...
    extras["tape_constructive_long"] = bool(tape_constructive_long)
    extras["tape_constructive_short"] = bool(tape_constructive_short)

    # Primary trigger must exist (otherwise we have nothing to anchor a plan).
    # NOTE: this is used by both PRE and CONFIRMED routing.
    primary_trigger = bool(vwap_event or rsi_event or macd_event or pro_trigger or early_pre_long or early_pre_short)
    ...
    pre_long_ok = bool(
        ...
        and isinstance(last_price, (float, int))
        and float(last_price) < float(ref_vwap)
        and (trigger_near or early_pre_long or (tape_mode and pre_trigger_near and tape_constructive_long))
        and (rsi_event or rsi_pre_long or macd_event or macd_pre_long or pro_trigger or (macd_build_long_pts >= 4))
        and (struct_pre_long or liquidity_event or orb_event or reclaim_attempt_long or tape_constructive_long)
        and (confirmation_score >= max(2, confirm_threshold - 1) or early_pre_long)
    )
    pre_short_ok = bool(
        isinstance(ref_vwap, (float, int))
        and isinstance(last_price, (float, int))
        and float(last_price) > float(ref_vwap)
        and (trigger_near or early_pre_short or (tape_mode and pre_trigger_near and tape_constructive_short))
        and (rsi_event or rsi_pre_short or macd_event or macd_pre_short or pro_trigger or (macd_build_short_pts >= 4))
        and (struct_pre_short or liquidity_event or orb_event or reclaim_attempt_short or tape_constructive_short)
        and (confirmation_score >= max(2, confirm_threshold - 1) or early_pre_short)
    )
...
def compute_ride_signal(
    ...
    **_ignored: object,
    ...
    if accept_components:
        accept_src = max(accept_component_weights, key=accept_component_weights.get)
        keys = [k for k, v in accept_components.items() if np.isfinite(v)]
        if keys:
            anchors = np.asarray([accept_components[k] for k in keys], dtype=float)
            weights = np.asarray([max(0.05, accept_component_weights.get(k, 0.05)) for k in keys], dtype=float)
            try:
                accept_line = float(np.average(anchors, weights=weights))
            except Exception:
                accept_line = float(accept_components.get(accept_src, anchors[0]))
        else:
            accept_line = float(accept_components.get(accept_src, ema20_ref if isinstance(ema20_ref, (float, int)) else last_price))
    else:
        accept_line = float(ema20_ref if isinstance(ema20_ref, (float, int)) else last_price)
        accept_src = "EMA20"
    if not np.isfinite(accept_line):
        if accept_src in accept_components and np.isfinite(accept_components.get(accept_src, np.nan)):
            accept_line = float(accept_components[accept_src])
        elif isinstance(ema20_ref, (float, int)) and np.isfinite(ema20_ref):
            accept_line = float(ema20_ref)
        else:
            accept_line = float(last_price)
    ...
    # Tape mode: constructive compression + directional pressure + release proximity.
    tape_constructive_score = 0.0
    tape_pressure_score = 0.0
    tape_release_score = 0.0
    tape_bad_chop = 0.0
    tape_mode_bonus = 0.0
    if tape_mode:
        try:
            rr_tail = (df["high"].astype(float) - df["low"].astype(float)).tail(6)
            rr_recent = float(rr_tail.tail(3).mean()) if len(rr_tail.tail(3)) else float(rr_tail.mean())
            rr_prior = float(rr_tail.head(3).mean()) if len(rr_tail.head(3)) else rr_recent
            body_tail = (df["close"].astype(float) - df["open"].astype(float)).abs().tail(6)
            body_recent = float(body_tail.tail(3).mean()) if len(body_tail.tail(3)) else float(body_tail.mean())
            body_prior = float(body_tail.head(3).mean()) if len(body_tail.head(3)) else body_recent
            wick_ratio = float((((df["high"].astype(float) - df["low"].astype(float)) - (df["close"].astype(float) - df["open"].astype(float)).abs()) / (df["high"].astype(float) - df["low"].astype(float)).replace(0, np.nan)).tail(3).mean())
            tightening_ok = bool(rr_recent <= 0.96 * max(rr_prior, 1e-9) and body_recent <= 1.02 * max(body_prior, 1e-9))
            hold_dir_ok = bool((direction == "LONG" and float(df["low"].tail(3).min()) >= float(df["low"].tail(6).min()) and close_q_long >= 0.48) or (direction == "SHORT" and float(df["high"].tail(3).max()) <= float(df["high"].tail(6).max()) and close_q_short >= 0.48))
            macd_vals = pd.to_numeric(df["macd_hist"], errors="coerce").dropna()
            if len(macd_vals) >= 3:
                if direction == "LONG":
                    pressure_ok = bool(float(macd_vals.iloc[-1]) > float(macd_vals.iloc[-2]) > float(macd_vals.iloc[-3]))
                else:
                    pressure_ok = bool(float(macd_vals.iloc[-1]) < float(macd_vals.iloc[-2]) < float(macd_vals.iloc[-3]))
            else:
                pressure_ok = False
            vol_tail = df["volume"].astype(float).tail(4)
            vol_build = bool(len(vol_tail) >= 3 and float(vol_tail.iloc[-1]) >= float(vol_tail.iloc[-3]))
            if direction == "LONG":
                release_ok = bool((last_price >= accept_line - 0.15 * float(atr_last)) and (last_price <= break_trigger_local if False else True))
            tape_constructive_score = 2.0 * float(tightening_ok) + 2.0 * float(hold_dir_ok) + 1.0 * float(compression_ok)
            tape_pressure_score = 2.0 * float(pressure_ok) + 1.0 * float(vol_build)
            tape_release_score = 0.0  # filled later in geometry chooser where break proximity is known
            tape_bad_chop = 2.0 if (np.isfinite(wick_ratio) and wick_ratio >= 0.72 and not vol_ok) else 0.0
            tape_mode_bonus = float(np.clip(tape_constructive_score + tape_pressure_score - tape_bad_chop, 0.0, 6.0))
        except Exception:
            tape_constructive_score = tape_pressure_score = tape_release_score = tape_bad_chop = tape_mode_bonus = 0.0
    ...
    pts += tape_mode_bonus if tape_mode else 0.0
    ...
    def _build_ride_entry_geometry(conviction_score: float) -> dict[str, object]:
        ...
        if actionable_local:
            ...
            tape_release_local = 0.0
            if tape_mode:
                tape_release_local += 2.0 if near_break_local else 0.0
                tape_release_local += 1.0 if breakout_acceptance_ok_local else 0.0
                tape_release_local += 1.0 if dist_pb_band_local > (0.70 * max(prox_dist_local, 1e-9)) else 0.0
                tape_release_local += 1.0 if ((direction == "LONG" and close_q_long >= 0.62) or (direction == "SHORT" and close_q_short >= 0.62)) else 0.0
            tape_breakout_bias = bool(tape_mode and (tape_constructive_score >= 3.0) and (tape_pressure_score >= 2.0) and (tape_release_local >= 3.0) and near_break_local and not stale_breakout_local and breakout_acceptance_ok_local)
            breakout_bias = bool(((breakout_confirmed_ok or breakout_elite_pre) and near_break_local and not stale_breakout_local and breakout_margin_ok) or tape_breakout_bias)
    ...
    if tape_mode and tape_mode_bonus > 0:
        why.append(f"Tape-ready (+{tape_mode_bonus:.0f})")
    ...
    return SignalResult(
        ...
        extras={
            ...
            "tape_constructive_score": float(tape_constructive_score),
            "tape_pressure_score": float(tape_pressure_score),
            "tape_mode_bonus": float(tape_mode_bonus),
